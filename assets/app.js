...
// Fallback local: se o Wikimedia estiver indisponível ou uma imagem externa
// falhar, o estudo ainda exibe uma ilustração bíblica leve gerada em SVG.
function imagemFallbackData(estudo) {
  const titulo = String(estudo?.titulo || "Estudo bíblico").replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
  const svg = `
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 675">
    <defs><linearGradient id="g" x1="0" x2="1" y1="0" y2="1"><stop stop-color="#5E1B22"/><stop offset="1" stop-color="#0B3B2E"/></linearGradient></defs>
    <rect width="1200" height="675" fill="url(#g)"/>
    <circle cx="950" cy="145" r="115" fill="#E3B23C" opacity=".22"/>
    <path d="M250 485 L600 405 L950 485 L600 560 Z" fill="#FFFCF5" opacity=".96"/>
    <path d="M600 405 L600 560" stroke="#D8B98A" stroke-width="8"/>
    <path d="M250 485 L600 405 L600 560 L250 485 Z" fill="#F2E9D8"/>
    <path d="M950 485 L600 405 L600 560 L950 485 Z" fill="#E9DDC5"/>
    <path d="M600 185 L600 355 M525 270 L675 270" stroke="#F2CB4E" stroke-width="24" stroke-linecap="round"/>
    <text x="600" y="95" text-anchor="middle" fill="#F2CB4E" font-family="Georgia,serif" font-size="30" letter-spacing="5">OUVINDO A VOZ DE DEUS</text>
    <text x="600" y="625" text-anchor="middle" fill="#FFFCF5" font-family="Arial,sans-serif" font-size="27">${titulo}</text>
  </svg>`;
...
async function buscarImagemEstudo(estudo) {
  ...
  for (const consulta of consultas) {
    try {
      ...
      const candidatos = paginas.map(pg => {
        ...
        // Muitas imagens do Commons são SVG, mas o thumburl fornecido pela API
        // é rasterizado (JPG/PNG). Não descarte a imagem só porque o arquivo
        // original é SVG.
        const imagemUrl = info.thumburl || info.url;
        if (!/^https?:\/\//i.test(imagemUrl)) return null;
  ...
  return { url: imagemFallbackData(estudo), credito: "Ilustração local de fallback", licenca: "gerada pelo aplicativo", paginaUrl: "#" };
}
...
function imagemEstudoEl(estudo) {
  ...
  if (cache.status === "ok") {
    ...
    const img = el("img", { class: "estudo-imagem", src: cache.url, alt: estudo.titulo, loading: "lazy", referrerpolicy: "no-referrer" });
    img.addEventListener("error", () => {
      if (!img.dataset.fallback) {
        img.dataset.fallback = "1";
        img.src = imagemFallbackData(estudo);
      }
    });
