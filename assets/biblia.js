...
function parseListaVersiculos(resto) {
  ...
  const encontrados = [];
  for (const bloco of resto.split(',').map((s) => s.trim()).filter(Boolean)) {
    const m = bloco.match(/^(\d+)(?:\s*-\s*(\d+))?$/);
    if (!m) continue;
    const ini = Number(m[1]);
    const fim = m[2] ? Number(m[2]) : ini;
    for (let n = ini; n <= fim; n++) encontrados.push(n);
  }
  return encontrados.length ? [...new Set(encontrados)].sort((a,b) => a-b) : null;
}
...
function parseReferencias(refString) {
  ...
  for (const parte of partes) {
    ...
    resultados.push({
      ...
      versiculosAlvo: parseListaVersiculos(resto),
      vIni: null,
      vFim: null,
    });
  }
...
async function buscarPassagem(citacao, versao) {
  ...
  if (citacao.versiculosAlvo?.length) {
    const alvo = new Set(citacao.versiculosAlvo);
    versiculos = versiculos.filter((v) => alvo.has(Number(v.number)));
  }
